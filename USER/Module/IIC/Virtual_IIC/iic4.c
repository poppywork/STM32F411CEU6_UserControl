...
#define SDA_READ   ((I2C_MONI_SDA_GPIO4->IDR & I2C_MONI_SDA_PIN4)?1:0)       // 不用改成输入模式就能读
 ...
 */
void AS5600_Set_TempZeroByReg_4(AS5600_Encoder_t *encoder, uint16_t new_zero_raw) {
    if (!encoder->is_initialized || !detect_magnet_4()) {
        return; // 未初始化或无磁铁，退出
    }
...
void AS5600_Update_4(AS5600_Encoder_t *encoder) {
    ...
    encoder->angle = get_angle_4(); // 读当前原始角度
